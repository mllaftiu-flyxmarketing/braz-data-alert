...
def set_telegram_general_message(message: str, chat_id: Optional[str] = None, thread_id: Optional[str] = None):
    try:
        target_chat_id = chat_id if chat_id else cfg.telegram_chat_id
        payload = {
        ...
        response = requests.post(cfg.telegram_send_general_message_url, data=payload, timeout=15)

        if response.status_code != 200:
            body = response.text
            error = f"Project: {cfg.project_name} | Error: status code {response.status_code} | Body: {body}"
        ...
        if not data or not data.get("ok", False):
            ...
            error = f"Project: {cfg.project_name} | Error: {desc}"
            raise Exception(error)

    except Exception as e:
        error = f"Project: {cfg.project_name} | Error: {str(e)}"
        raise Exception(error)
...
def set_telegram_topic_message(
    ...
    try:
        ...
        use_thread_id = thread_id if thread_id is not None else cfg.telegram_topic_id
        if use_thread_id and str(use_thread_id).strip() != "":
            payload["message_thread_id"] = str(use_thread_id)
        else:
            use_reply_id = reply_to_message_id if reply_to_message_id is not None else cfg.telegram_topic_reply_to_message_id
            if use_reply_id and str(use_reply_id).strip() != "":
                payload["reply_to_message_id"] = str(use_reply_id)

        headers = {"Content-Type": "application/json"}
        response = requests.post(
            cfg.telegram_send_topic_message_url,
            json=payload,
            headers=headers,
            timeout=15,
        )
