    ...
    try:
        ...
        for raw in present_rows:
            ...
            if isinstance(d, datetime):
                ...
                present_dates.add(datetime.strptime(str(d), "%Y-%m-%d").date())
        ...
        set_log(str(e), reason="Error", method="get_sessions_missing_dates_results")
        return []


def get_sessions_ids_by_dates(table_name: str, dates: list) -> dict:
    ids_map: Dict[str, list] = {}
    try:
        if not dates:
            return ids_map
        ...
        set_log(str(e), reason="Error", method="get_sessions_ids_by_dates")
        return {}